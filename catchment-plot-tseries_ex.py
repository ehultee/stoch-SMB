...
best_n = fit_catchment_series(s)
for m in model_names:
    mod = AutoReg(s[m], best_n, trend='ct', seasonal=True).fit()
    fv = mod.fittedvalues
    r = mod.resid
    mod_fits[m] = fv
    mod_resids[m] = r
...
for i,m in enumerate(model_names):
    if 'NHM' in m:
        ax2.plot(s[m], label=m, color=colors_w[i])
...
for i,m in enumerate(model_names):
    ax3.plot(s[m], label=m, color=colors_w[i])
    if 'NHM' in m:
